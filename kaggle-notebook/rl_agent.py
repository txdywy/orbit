"""
Orbit Wars RL Agent - Kaggle Notebook
Train and submit an RL-enhanced agent.
"""
...
def agent(obs):
    """Main agent function - Heuristic with RL-inspired adjustments"""
    ...
    for source in sorted(my_planets, key=lambda p: (p.ships, p.production), reverse=True):
        ...
        # Dynamic reserve based on game state
        if step < 40:
...
print("- Focus enemy targeting")
print("- Endgame defense")
print("- Aggressive opening")
print("- Comet handling")
...
final = env.steps[-1]
print(f"\nQuick test: Player 0 reward = {final[0].reward}")
