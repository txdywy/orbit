...
try:
    ...
    print("PyTorch not available, using heuristic only")
...
def fleet_hits_planet(fleet, planet):
    ...
    end = (fleet.x + math.cos(fleet.angle) * 130.0, fleet.y + math.sin(fleet.angle) * 130.0)
...
# RL Training Code (if PyTorch available)
if TORCH_AVAILABLE:
    ...
    def extract_state_features(obs):
        """Extract features from observation for RL"""
        player = get(obs, "player", 0)
        planets = parse_planets(get(obs, "planets", []))
        fleets = [Fleet(*f) for f in get(obs, "fleets", [])]
        step = int(get(obs, "step", 0) or 0)
        ...
        enemy_ships = sum(p.ships for p in enemy_planets) + sum(f.ships for f in fleets if f.owner not in (-1, player))
    ...
    print("RL training code ready. Use Kaggle GPU for training.")
else:
    print("PyTorch not available. Install torch for RL training.")

print("Agent code loaded successfully!")
print(f"Current agent: Heuristic-based with net value calculation")
print(f"To train RL model: Run this notebook on Kaggle with GPU enabled")
